...
int rtd_periodic_callback(char *ptr, void *args) {
    ...
    if (ptr != NULL) {
        CLOG("exiting from rtd sensor periodic read %s\n", ptr);
        return 1;
    }
    ...
    // int test = 0;
    // char print_str[80] = {0};
    spi_t *spi = max_spi_init(rtd_spi_dev, SPI_MODE_1, MAX_SPI_FREQ);
    ...
    for (channel = 1; channel <= 4; channel++) {
        if(st_rtd_read_loc->IO_channel == 0 || st_rtd_read_loc->IO_channel == channel){
            ...
            Max_single_byte_write(spi, WRITE_CONFIGURATION_ADDR, 0x00);

            if (!(Max_sensor_scan(spi, st_rtd_read_loc, channel))) {
                ...
                st_rtd_read_loc->rtd_fault_status[channel - 1] = Max_fault_detection(spi);
                CLOG("Error Occured While Reading RTD Sensor\r\n");
                CLOG("Fault Reg : %d ", st_rtd_read_loc->rtd_fault_status[channel - 1]);
                CLOG("=> b\'");
                for (index = 0; index < 8; index++) {
                    CLOG("%d", ((st_rtd_read_loc->rtd_fault_status[channel - 1] >> (7 - index)) & 0x01));
                }

                if (st_rtd_read_loc->rtd_fault_status[channel - 1] & 0x80) {
                    CLOG("Bit D7 is Set\r\n");
                    CLOG("Condition Detected :- Measured Resistance Greater than High Fault Threshold Value\n");
                    CLOG("Possible Causes: \n\t1. Open RTD Element \n\t2.RTD+ shorted high and not connected to RTD \n\t3.FORCE+ shorted high and connected to RTD\n");
                }
                if (st_rtd_read_loc->rtd_fault_status[channel - 1] & 0x40) {
                    CLOG("Bit D6 is Set\r\n");
                    CLOG("Condition Detected :- Measured Resistance less than low fault threshold value\r\n");
                    CLOG("Possible Causes:- \n\t1. RTD+ shorted to RTD- \n\t2. RTD+ shorted low and not connected to RTD \n\t3. FORCE+ shorted low\r\n");
                }
                if (st_rtd_read_loc->rtd_fault_status[channel - 1] & 0x20) {
                    CLOG("Bit D5 is Set\r\n");
                    CLOG("Condition Detected :- V(REFIN-) > 0.85*V(BIAS)\r\n");
                ...
                if (st_rtd_read_loc->rtd_fault_status[channel - 1] & 0x10) {
                    CLOG("Bit D4 is Set\r\n");
                    CLOG("Condition Detected :- V(REFIN-) < 0.85*V(BIAS). FORCE- open\r\n");
                    CLOG("Possible Causes:- \n\t1. RTD- shorted low\r\n");
                }
                if (st_rtd_read_loc->rtd_fault_status[channel - 1] & 0x08) {
                    CLOG("Bit D3 is Set\r\n");
                    CLOG("Condition Detected :- V(RTDIN-) < 0.85*V(BIAS). FORCE- open\r\n");
                    CLOG("Possible Causes:- \n\t1. FORCE+ shorted low \n\t2. RTDIN+ shorted low and connected to RTD \n\t3. RTDIN- shorted low\r\n");
                }
                if (st_rtd_read_loc->rtd_fault_status[channel - 1] & 0x04) {
                    CLOG("Bit D2 is Set\r\n");
                    CLOG("Condition Detected :- OverVoltage or UnderVoltage Fault\r\n");

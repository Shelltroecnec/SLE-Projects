 ...
 * 
 * @param spi           SPI handler
 * @param address       Register addres, address shuold be 4-bit only 
 ...
 */
int ltc2634_dac_read_write(spi_t *spi, uint8_t address, uint8_t operation, uint16_t* data1) {
    // uint8_t count = 0;
...
int ltc2634_dac_set_output(spi_t *spi, uint8_t channel, float voltage) {
    ...
    //data = (uint16_t)(address/LTC2634_BITS_RES)*(2.5); //Calculting the Ideal Vout
    data = (uint16_t)((LTC2634_BITS_RES * voltage) / LTC2634_REF_VOLT);
    

    CLOG("DAC Output: BC = %d   Volt = %0.3f\n", data, voltage);
    CLOG("Setting DAC O/P: %d, addr: %x\n", data, address);

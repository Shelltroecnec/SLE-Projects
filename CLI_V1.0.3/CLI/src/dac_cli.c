...
void dac_cmd(int argc, char **argv, void *additional_arg)
{
    ...
    if (out_value > 2.5f || out_value < 0.0f)
    {
        CLOG("Invalid voltate output range!!!\n");
        dac_help();
        return;
    }
    ...
    ltc2634_dac_deinit(spi_dac); // frees spaces and peripheral after reading
}
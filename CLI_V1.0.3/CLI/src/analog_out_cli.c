...
void analog_out_help(void) {
    ...
    CLOG("           3 - ouput channel 3\n");
    CLOG("           4 - ouput channel 4\n");
...
int aout_callback(char *ptr, void *args) {
    ...
    if (ltc2634_dac_set_output_cnt(pst_analog_out_cb->spi, pst_analog_out_cb->aout_channel, pst_analog_out_cb->aout_dac_curr_bc) > -1) {
        CLOG("DAC Output BC = %d   Volt = %0.3f\n", pst_analog_out_cb->aout_dac_curr_bc,
             ((float)(LTC2634_REF_VOLT * pst_analog_out_cb->aout_dac_curr_bc) / LTC2634_BITS_RES));
    } else {
...
void analog_out_cmd(int argc, char **argv, void *additional_arg) {
    ...
    if ((channel > 4 || channel < 0) && channel == 2) {
    ...
    if ((spi_dac = ltc2634_dac_init(config.ltc2634_spi_device, SPI_MODE_3, DAC_SPI_FREQ)) == NULL) {
    ...
    if (dac_output_state == OUTPUT_RAMP) {
        ...
        if (ltc2634_dac_set_output(spi_dac, channel, out_value) > -1) {
            ILOG("Successfully Updated the DAC Output\n");
        } else {
            WLOG("Fail to update the DAC Output!!!\n");
        }
    }
    ltc2634_dac_deinit(spi_dac);
}